...
Begin VB.Form frmRecGrp 
   ...
   Begin VB.Menu mnuRequestRecGrp 
      Caption         =   "&Request Record Group"
      Begin VB.Menu mnuAddRequestRecGrp 
         Caption         =   "&Add Request Record Group"
      End
...
On Error GoTo Err_WriteSirInfo
  ...
  If FileExists(SirTemplate) Then
    ...
    strMsg = "Request Record Group Template successfully written. Do you want to create " & _
        "another Template?"
    strTitle = "Request Record Group Template"
    style = vbYesNo
  ...
  Else
    ...
    'file not found
    MsgBox SirTemplate & " does not exist! Please verify the SIR number before adding your template.", vbOKOnly, "Request Record Group"
...
'****************************************************************************
' Name: txtPrefix_LostFocus
' Purpose: Checks to see that the user has entered a prefix without special
'           and is four or fewer characters.
'
' Created by: Christina Mitchell
' Date Created: 09/22/97
'****************************************************************************
...
On Error GoTo Err_txtPrefix_LostFocus
    ...
    Do While intI <= Len(strTemp)
        ...
        If (intAscNumber >= 33 And intAscNumber <= 47) Or (intAscNumber >= 58 And intAscNumber <= 64) Or (intAscNumber >= 91 And intAscNumber <= 96) Or (intAscNumber >= 123 And intAscNumber <= 127) Then
            MsgBox "Please enter a COBOL Prefix that does not contain special characters.", vbOKOnly, "Request Record Group Template"
            
            'return focus to txtPrefix and highlight entry
            txtPrefix.SetFocus
            txtPrefix.SelStart = 0
            txtPrefix.SelLength = Len(strTemp)
...
Exit_txtPrefix_LostFocus:
    Exit Sub
    
Err_txtPrefix_LostFocus:
    MsgBox Error$
    Resume Exit_txtPrefix_LostFocus
...
On Error GoTo Err_txtSirNo_Change
    ...
    If Len(txtSirNo.Text) > 0 Then
        ...
        If Not IsNumeric(strTemp) Then
            MsgBox "Please enter a five digit numeric value.", vbOKOnly, "Request Record Group Template"
